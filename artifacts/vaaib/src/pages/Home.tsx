...
export default function Home() {
  ...
  const onSubmit = async (data: ContactFormData) => {
    ...
    toast({
      ...
      description: "We'll be in touch shortly to discuss your AI visibility strategy.",
    });
    reset();
  };
              ...
              {[
                {
                  icon: Search,
                  stat: "67%",
                  text: "of consumers now use AI chat tools for product and service discovery instead of traditional search."
                },
                {
                  icon: Clock,
                  stat: "6+ Mos",
                  text: "is how long traditional SEO takes to show results. AI indexing happens in days or hours."
                },
                {
                  icon: TrendingUp,
                  stat: "90%",
                  text: "of search decisions will be driven by AI-powered answers within the next 24 months."
                }
              ].map((item, i) => (
                <motion.div 
                  key={i} 
                  variants={fadeUp}
                  className="glass-panel p-8 rounded-2xl hover:border-primary/30 transition-colors duration-500 group"
                >
          ...
          <div className="max-w-7xl mx-auto">
            ...
            >
              {[
                {
                  icon: Globe,
                  title: "AI-Indexed Content Strategy",
                  desc: "We structure your site's data precisely how Large Language Models prefer to ingest it, ensuring you are recommended when users ask AI for your services."
                },
                {
                  icon: Activity,
                  title: "Real-Time Visibility Monitoring",
                  desc: "Track how often your business is cited in AI responses across different platforms with our proprietary monitoring dashboard."
                },
                {
                  icon: MessageSquare,
                  title: "Chatbot Integration Ready",
                  desc: "Turn your website into an interactive agent. Deploy custom-trained AI assistants that know your business inside and out."
                },
                {
                  icon: Mic,
                  title: "Optimized for Voice & AI Search",
                  desc: "Move beyond keywords. We optimize for the conversational, natural language queries people use when talking to AI."
                }
              ].map((feature, i) => (
                <motion.div 
                  key={i} 
                  variants={fadeUp}
                  className="p-8 rounded-2xl bg-gradient-to-br from-white/[0.03] to-transparent border border-white/5 hover:bg-white/[0.05] transition-all flex gap-6"
                >
            ...
            >
              ...
              <motion.div variants={fadeUp} className="glass-panel p-8 rounded-3xl h-fit">
                ...
                <ul className="space-y-4 mb-8">
                  {["1-page responsive website", "AI-optimized content structure", "Basic SEO metadata", "Contact form integration", "30-day post-launch support"].map((feature, i) => (
                ...
                <Button variant="outline" className="w-full rounded-xl h-12" onClick={() => scrollTo('contact')}>Select Starter</Button>
              </motion.div>
              ...
              <motion.div variants={fadeUp} className="relative p-8 rounded-3xl bg-gradient-to-b from-card to-background border-2 border-primary shadow-[0_0_40px_rgba(79,70,229,0.15)] md:-my-4 z-10">
                ...
                <ul className="space-y-4 mb-8">
                  {["Up to 5 pages", "Advanced AI content optimization", "Full traditional SEO suite", "Blog/news CMS section", "Analytics integration setup", "90-day post-launch support"].map((feature, i) => (
                    <li key={i} className="flex items-start gap-3 text-sm">
                      <CheckCircle2 className="w-5 h-5 text-primary shrink-0" />
                ...
                <Button className="w-full rounded-xl h-12 text-base" onClick={() => scrollTo('contact')}>Select Pro</Button>
              </motion.div>
              ...
              <motion.div variants={fadeUp} className="glass-panel p-8 rounded-3xl h-fit">
                ...
                <ul className="space-y-4 mb-8">
                  {["Unlimited pages", "Custom AI Chatbot integration", "Automated AI content pipeline", "Voice search optimization", "Priority indexing strategy", "Custom AI personas", "6-month support & reviews"].map((feature, i) => (
                    <li key={i} className="flex items-start gap-3 text-sm">
                      <CheckCircle2 className="w-5 h-5 text-primary shrink-0" />
                      <span className="text-foreground/80">{feature}</span>
                    </li>
                ...
                <Button variant="outline" className="w-full rounded-xl h-12" onClick={() => scrollTo('contact')}>Select Authority</Button>
              </motion.div>
            </motion.div>
          </div>
        </section>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea 
                    ...
                    placeholder="Tell us about your current online presence..."
                  />
